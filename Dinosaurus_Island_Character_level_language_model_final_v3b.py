...
def sample(parameters, char_to_ix, seed):
    ...
    n_a = Waa.shape[1]
#     print((len(char_to_ix), 1))
    ### START CODE HERE ###
    # Step 1: Create the a zero vector x that can be used as the one-hot vector 
    # representing the first character (initializing the sequence generation). (≈1 line)
    x = np.zeros(shape=(len(char_to_ix), 1))#shape=(T_x,1))
    # Step 1': Initialize a_prev as zeros (≈1 line)
    a_prev = np.zeros(shape=(n_a, 1))
    
    ####### x,a are one hot column vector to represent one char only
    ...
    while (idx != newline_character and counter != 50):
        ...
        a = np.tanh(np.dot(Wax, x) + np.dot(Waa, a_prev) + b) # a⟨t+1⟩=tanh(Waxx⟨t+1⟩+Waaa⟨t⟩+b)
        z = np.dot(Wya, a) + by # z⟨t+1⟩=Wyaa⟨t+1⟩+by
        y = softmax(z)
        ...
        idx = np.random.choice(range(len(char_to_ix)), p=y.ravel())
...
def model(data, ix_to_char, char_to_ix, num_iterations = 35000, n_a = 50, dino_names = 7, vocab_size = 27, verbose = False):
    ...
    for j in range(num_iterations):
        ...
        X = [None] + single_example_ix #rnn_forward(X, Y, a_prev, parameters)
